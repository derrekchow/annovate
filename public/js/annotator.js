(function (f) { if (typeof exports === "object" && typeof module !== "undefined") { module.exports = f() } else if (typeof define === "function" && define.amd) { define([], f) } else { var g; if (typeof window !== "undefined") { g = window } else if (typeof global !== "undefined") { g = global } else if (typeof self !== "undefined") { g = self } else { g = this } g.annotator = f() } })(function () {
			...
			*/
			...
			HttpStorage.prototype._onError = function (xhr) {
				...
				if (xhr.status === 400) {
					...
					message = _t(`Error ${xhr.status}: Unknown error while speaking to annotation store!`);
				}
			...
			var Editor = exports.Editor = Widget.extend({
				...
				constructor: function (options) {
					...
					this.element
						...
						.on('mouseover.' + NS, '.annotator-tag', function(e) {
							e.target.focus();
						})
			...
			var Viewer = exports.Viewer = Widget.extend({
				...
				load: function (annotations, position) {
					...
					var secondMaxType = null;
					for(var type in count){
						if(type !== maxType && count[type] > secondMaxCount){
							secondMaxCount = count[type];
							secondMaxType = type;
						}
					}
					var thirdMaxType = Object.keys(count).filter((value, index, array) => {
						if (value !== maxType && value !== secondMaxType){
							return true;
						} else {
							return false;
						}
					})[0]
					var sorted_annotations = this.annotations.slice()
					sorted_annotations.sort((a, b) => {
						if(a.tags[0].toLowerCase() === maxType){
							return -1;
						} else if (a.tags[0].toLowerCase() == secondMaxType && b.tags[0].toLowerCase() !== maxType){
							return -1;
						} else {
							return 1;
						}
					});
					for (var i = 0, len = sorted_annotations.length; i < len; i++) {
						var annotation = sorted_annotations[i];
						var type = annotation.tags[0].toLowerCase();
						if(!seen[type]){
							seen[type] = true;
